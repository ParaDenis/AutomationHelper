    ...
    public class GoogleDriveHelper
    {
        ...
        private File GetLastCreatedFolderIn(string folderID)
        {
            return
                GetFileList(folderID)
                    .Where(f => f.MimeType.Contains("folder"))
                    .OrderBy(f => f.CreatedDate ?? new DateTime(2010, 10, 10))
                    .Last("Can't find any build folders at google drive");
        }
     ...
     /*   internal void DownloadLastBuild()
       ...
       /* public void UploadLastBuild()
        ...
        private File createDirectory(string _title, string _description, string _parentId)
        {
        ...
        private void uploadFile(string _uploadFile, string _parentId)
        {
            Wait.UntilNumberOfExceptions(() =>
            {
                File res = null;
                ...
                for (int i = 0; i < numberOfAttempts; i++)
                {
                    if (System.IO.File.Exists(_uploadFile))
                    {
                        ...
                        var body = new File
                        {
                            Title = Path.GetFileName(_uploadFile),
                            MimeType = mimeType,
                            Parents = new List<ParentReference>() {new ParentReference() {Id = _parentId}},
                        };
                        // File's content.
                    ...
                    else throw new FileNotFoundException(_uploadFile);
                    if (res.FileSize == new FileInfo(_uploadFile).Length)
                    {
                        //Log.InfoFormat("Succeded to upload file: {0} from {1} attempt", _uploadFile, i + 1);
                        break;
                    }
                    else
                    {
